...
Current wearable activity recognition systems are able to unobtrusively capture and recognize a person's activities throughout the whole day. These systems often rely on inertial sensor data that is captured by wearable sensors attached to the body to identify the activities performed. However, it is a great challenge to identify fine-grained activities just by using this single modality. In this paper, we use an accelerometer-based activity recognition system, the wrist-worn activity logger called HedgeHog\footnote{\url{http://www.ess.tu-darmstadt.de/hedgehog}} and investigate the possibility of integrating a capacitive proximity sensor unobtrusively into the wristband to obtain higher recognition rates for several activities in daily life.
...
Berlin et al. \cite{berlin_laerhoven_tei_2010} have augmented an accelerometer based sensor node with an RFID bracelet to be able to detect objects a human is interacting with. Thus, activities can use this additional modality 

\begin{figure}[htbp]
	\centering
		\includegraphics[width=1.00\textwidth]{Images/modes.png}
	\caption{Capacitive proximity sensing modes \cite{Smith1996}}
	\label{fig:modes}
\end{figure}
